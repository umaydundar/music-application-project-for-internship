...
async function displayEventsList() {
    const response = await fetch('/all_live_events/?username='+ username);
    const topResponse = await fetch('/top_live_events/?username='+ username);
    const favResponse = await fetch('/fav_live_events/?username='+ username);
    const data = await response.json();
    const dataTop = await topResponse.json();
    const dataFav = await favResponse.json();
    const events = data.live;
    const topEvents = dataTop.live;
    const favEvents = dataFav.live;
    let eventCount = 0;

    async function createEventElement(event, className) {
        ...
        joinEventButton.className = "join-event-btn";
        joinEventButton.textContent = "Join Event";
        joinEventButton.onclick = () => {ListenLive(eventCount++)}

        const quitEventButton = document.createElement('button');
        quitEventButton.className = "quit-event-btn";
        quitEventButton.textContent = "Quit Event";
        quitEventButton.onclick = () => {QuitLive(eventCount)}

        eventContentDiv.append(eventName);
        eventContentDiv.append(eventInfo);
        eventContentDiv.append(eventArtist);
        eventContentDiv.append(joinEventButton);
        eventContentDiv.append(quitEventButton);
        eventImageDiv.appendChild(eventImage);
        eventDiv.append(eventImageDiv);
        eventDiv.append(eventContentDiv);
    ...
    for (const event of events) {
        let artistName = await getArtistName(event.artist_id);
        const eventElement = await createEventElement(event, 'event-all', artistName);
        document.getElementsByClassName('all')[0].appendChild(eventElement);
    }

    for (const event of topEvents) {
        let artistName = await getArtistName(event.artist_id);
        const eventElement = await createEventElement(event, 'event-top', artistName);
        document.getElementsByClassName('top')[0].appendChild(eventElement);
    }

    for (const event of favEvents) {
        let artistName = await getArtistName(event.artist_id);
        const eventElement = await createEventElement(event, 'event-fav', artistName);
...
async function ListenLive(buttonId){
    document.getElementsByClassName("join-event-btn")[buttonId].style.display = "none";
    document.getElementsByClassName("quit-event-btn")[buttonId].style.display = "flex";
}

async function QuitLive(buttonId){
    document.getElementsByClassName("quit-event-btn")[buttonId].style.display = "none";
    document.getElementsByClassName("join-event-btn")[buttonId].style.display = "flex";
}


async function listenLiveEvent(){

}

async function quitListenLiveEvent(){
